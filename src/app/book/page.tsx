...
export default function BookingFlow() {
  ...
  return (
    <div className="min-h-screen bg-[#f0f0f5] pb-24 md:pb-0">
      <header className="sticky top-0 z-50 bg-white shadow-sm h-16 flex items-center px-4">
        ...
        <div className="flex-1"><h1 className="font-bold text-[#282c3f]">Book a Service</h1><p className="text-xs text-[#686b78]">Step {step} of 4</p></div>
      </header>
      ...
      <div className="container mx-auto max-w-5xl py-6 px-4 grid md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-4">
          ...
          <Card className={`border-none shadow-sm ${step!==1?"opacity-50 pointer-events-none":""}`}>
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3"><div className="w-8 h-8 rounded bg-[#282c3f] text-white flex items-center justify-center font-bold text-sm">1</div><h2 className="text-lg font-bold">Select Service</h2></div>
                {step>1 && <CheckCircle2 className="text-green-600 h-6 w-6"/>}
              </div>
              {step===1 && <div className="space-y-3">
                {services.map(s=>(
                  <div key={s.id} onClick={()=>setSelectedService(s.id)} className={`border p-4 rounded-xl flex justify-between items-center cursor-pointer transition-all ${selectedService===s.id?"border-[#fc8019] bg-orange-50":"hover:border-orange-300"}`}>
                    <div><h3 className="font-bold text-[#282c3f]">{s.name}</h3><p className="text-sm text-[#686b78]">{s.description_short}</p></div>
                    <div className="text-right"><p className="font-bold text-[#282c3f]">₹{s.discounted_price||s.price}</p>{s.discounted_price && <p className="text-xs text-[#686b78] line-through">₹{s.price}</p>}</div>
                  </div>
                ))}
                <Button className="w-full bg-[#fc8019] hover:bg-[#e57212] text-white font-bold h-12" onClick={()=>{if(selectedService)setStep(2);}} disabled={!selectedService}>Continue <ChevronRight className="ml-2 h-5 w-5"/></Button>
              </div>}
            </CardContent>
          </Card>
          ...
          <Card className={`border-none shadow-sm ${step!==2?"opacity-50 pointer-events-none":""}`}>
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3"><div className="w-8 h-8 rounded bg-[#282c3f] text-white flex items-center justify-center font-bold text-sm">2</div><h2 className="text-lg font-bold">Who are we pampering?</h2></div>
                {step>2 && <CheckCircle2 className="text-green-600 h-6 w-6"/>}
              </div>
              {step===2 && <div className="space-y-4">
                {pets.length>0 && !showAddPet && <div className="grid grid-cols-2 gap-3">
                  {pets.map(p=>(
                    <div key={p.id} onClick={()=>setSelectedPet(p.id)} className={`border p-4 rounded-xl flex flex-col items-center gap-2 cursor-pointer ${selectedPet===p.id?"border-[#fc8019] bg-orange-50":"hover:border-orange-300"}`}>
                      <div className="w-14 h-14 rounded-full bg-orange-100 flex items-center justify-center"><PawPrint className="h-6 w-6 text-[#fc8019]"/></div>
                      <h3 className="font-bold text-sm">{p.name}</h3><p className="text-xs text-[#686b78]">{p.breed||p.type}</p>
                    </div>
                  ))}
                  <div onClick={()=>{setShowAddPet(true);setSelectedPet("");}} className="border border-dashed p-4 rounded-xl flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-gray-50">
                    ...
                    <h3 className="font-bold text-sm text-[#686b78]">Add New Pet</h3>
                  </div>
                </div>}

                {(showAddPet || pets.length===0) && <div className="space-y-3 border rounded-xl p-4 bg-gray-50">
                  <h3 className="font-bold text-[#282c3f]">Add Pet Details</h3>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <label htmlFor="neutered" className="text-sm text-[#686b78]">Neutered / Spayed</label>
                  </div>
                  <div className="flex gap-3">
                    {pets.length>0 && <Button variant="outline" className="flex-1 h-11" onClick={()=>setShowAddPet(false)}>Cancel</Button>}
                    <Button className="flex-1 h-11 bg-[#fc8019] hover:bg-[#e57212] text-white font-bold" onClick={savePet} disabled={!petForm.name||petSaving}>
                ...
                {!showAddPet && <div className="flex gap-3">
                  ...
                  <Button className="w-2/3 bg-[#fc8019] hover:bg-[#e57212] text-white font-bold h-12" onClick={()=>{if(selectedPet)setStep(3);}} disabled={!selectedPet}>Continue</Button>
                </div>}
              </div>}
            </CardContent>
          </Card>
          ...
          <Card className={`border-none shadow-sm ${step!==3?"opacity-50 pointer-events-none":""}`}>
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3"><div className="w-8 h-8 rounded bg-[#282c3f] text-white flex items-center justify-center font-bold text-sm">3</div><h2 className="text-lg font-bold">Address & Time Slot</h2></div>
                {step>3 && <CheckCircle2 className="text-green-600 h-6 w-6"/>}
              </div>
              {step===3 && <div className="space-y-5">
                <div className="space-y-3">
                  <h3 className="font-bold text-sm text-[#686b78] uppercase tracking-wider">Your Address</h3>
                ...
                <div className="space-y-3">
                  <h3 className="font-bold text-sm text-[#686b78] uppercase tracking-wider">Select Date</h3>
                  <div className="flex overflow-x-auto gap-3 pb-2 hide-scrollbar">
                    {days.map((d,i)=>(
                      <div key={i} onClick={()=>setSelectedDate(i)} className={`min-w-[100px] py-3 text-center rounded-lg border cursor-pointer text-sm ${selectedDate===i?"border-[#fc8019] text-[#fc8019] font-bold bg-orange-50":"text-[#686b78] hover:border-orange-300"}`}>
                ...
                <div className="space-y-3">
                  <h3 className="font-bold text-sm text-[#686b78] uppercase tracking-wider">Select Time</h3>
                  <div className="grid grid-cols-4 gap-3">
                    {TIME_SLOTS.map(t=>(
                      <div key={t} onClick={()=>setSelectedTime(t)} className={`py-2.5 text-center rounded-lg border text-sm cursor-pointer ${selectedTime===t?"border-[#fc8019] text-[#fc8019] font-bold bg-orange-50":"text-[#686b78] hover:border-orange-300"}`}>{t}</div>
                    ))}
                  </div>
                </div>
                <div className="flex gap-3">
                  <Button variant="outline" className="w-1/3 h-12 font-bold" onClick={()=>setStep(2)}>Back</Button>
                  <Button className="w-2/3 bg-[#fc8019] hover:bg-[#e57212] text-white font-bold h-12" onClick={()=>{if(address.line1&&address.city&&selectedTime)setStep(4);}} disabled={!address.line1||!address.city||!selectedTime}>Continue</Button>
                </div>
              </div>}
            </CardContent>
          </Card>
        ...
        <div className="md:col-span-1">
          <div className="sticky top-24">
            <Card className="border-none shadow-sm overflow-hidden">
              <div className="bg-[#282c3f] text-white p-4"><h2 className="font-bold text-lg">Booking Summary</h2></div>
              <CardContent className="p-0">
                <div className="p-4 space-y-3 border-b border-dashed">
                  {svc?<div className="flex justify-between"><div><h3 className="font-bold text-[#282c3f]">{svc.name}</h3>{selectedPet&&<p className="text-xs text-[#686b78] mt-1">For {pets.find(p=>p.id===selectedPet)?.name}</p>}</div><span className="font-bold">₹{price}</span></div>:<p className="text-sm text-[#686b78]">Select a service</p>}
                  {selectedTime&&<p className="text-sm text-[#686b78] flex items-center gap-1"><Clock className="h-3 w-3"/>{selectedDate===0?"Today":selectedDate===1?"Tomorrow":fmtDay(days[selectedDate])}, {selectedTime}</p>}
                  {address.line1&&<p className="text-sm text-[#686b78] flex items-center gap-1"><MapPin className="h-3 w-3"/>{address.line1}, {address.city}</p>}
                </div>
                {svc&&<div className="p-4 bg-gray-50 space-y-2">
                  <div className="flex justify-between text-sm"><span className="text-[#686b78]">Subtotal</span><span>₹{price}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-[#686b78]">GST (18%)</span><span>₹{tax}</span></div>
                ...
                {step===4&&<div className="p-4 space-y-3">
                  ...
                  <Button className="w-full bg-[#fc8019] hover:bg-[#e57212] text-white font-bold h-12 text-lg" onClick={placeBooking} disabled={submitting}>
